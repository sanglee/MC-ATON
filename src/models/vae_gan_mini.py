...
class Encoder(nn.Module):
    def __init__(self, input_channel):
        ...
        self.conv1 = nn.Conv2d(input_channel, 64, 5, padding=2, stride=2)  # in_channels=3
...
class Decoder(nn.Module):
    def __init__(self, output_channel):
        ...
        self.bn1 = nn.BatchNorm1d(4 * 4 * 256, momentum=0.9)
        self.relu = nn.LeakyReLU(0.2)
        ...
        self.deconv3 = nn.ConvTranspose2d(128, 32, 6, stride=2, padding=2) # 32 for FMNIST, 64 for CIFAR10
        self.bn4 = nn.BatchNorm2d(32, momentum=0.9)
        self.deconv4 = nn.ConvTranspose2d(32, output_channel, 5, stride=1, padding=2)
        self.tanh = nn.Tanh()
...
class VAE_2d(nn.Module):
    def __init__(self, device, in_dim=3):
    ...
    def reparameterize(self, mean, logvar):
        ...
        epsilon = Variable(torch.randn(mean.size(0), 128)).to(self.device)
        z = mean + std * epsilon
        return z

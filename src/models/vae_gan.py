...
class Encoder(nn.Module):
    def __init__(self, channel_in=3, z_size=128, latent_dim=8):
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=latent_dim * latent_dim * self.size, out_features=1024, bias=False),
...
class Decoder(nn.Module):
    def __init__(self, z_size, size, latent_dim, out_dim):
        super(Decoder, self).__init__()
        # start from B*z_size
        self.latent_dim = latent_dim
        self.fc = nn.Sequential(nn.Linear(in_features=z_size, out_features=latent_dim * latent_dim * size, bias=False),
                                nn.BatchNorm1d(num_features=latent_dim * latent_dim * size, momentum=0.9),
                                nn.ReLU(True))
        self.size = size
        layers_list = []
        ...
        layers_list.append(nn.Sequential(
            nn.Conv2d(in_channels=self.size, out_channels=out_dim, kernel_size=5, stride=1, padding=0),
            nn.Tanh()
        ))
    ...
    def forward(self, ten):
        ten = self.fc(ten)
        ten = ten.view(len(ten), -1, self.latent_dim, self.latent_dim)
        ten = self.conv(ten)
        return ten
...
class VaeGan_pert(nn.Module):
    def __init__(self, classifier, device, latent_dim=8, z_size=128, recon_level=3):
        super(VaeGan_pert, self).__init__()
        # latent space size
        self.z_size = z_size
        self.device = device
        self.encoder = Encoder(channel_in=recon_level, z_size=self.z_size, latent_dim=latent_dim)
        self.decoder = Decoder(z_size=self.z_size, size=self.encoder.size, latent_dim=latent_dim, out_dim=recon_level)
    ...
    def forward(self, x, gen_size=10, zeta=None):
        if self.training:
            ...
            disc_class_tilde = self.discriminator(x_tilde)
            disc_class_gen = self.discriminator(x_p)
            disc_class = self.discriminator(x)
            disc_class = F.sigmoid(disc_class)
            disc_class_gen = F.sigmoid(disc_class_gen)
            disc_class_tilde = F.sigmoid(disc_class_tilde)
            ...
            return x_tilde, x_p, disc_class_tilde, disc_class_gen, disc_class, mus, log_variances
        else:
            if x is None:
    ...
    def __call__(self, *args, **kwargs):
        return super(VaeGan_pert, self).__call__(*args, **kwargs)

    @staticmethod
    def loss(x, x_tilde, disc_class_tilde, disc_class_sampled, disc_class, mus, log_variances): # x_tilde, x_p, disc_class_tilde, disc_class_gen, disc_class, mus, log_variances

        # reconstruction error
        # reconle = 0.5 * (x.view(len(x), -1) - x_tilde.view(len(x_tilde), -1)) ** 2
        reconle = torch.mul(x - x_tilde, x - x_tilde)
        # kl divergence
        kl = -0.5 * torch.sum(-log_variances.exp() - torch.pow(mus, 2) + log_variances + 1, 1)
        # decoder loss
        # decle = - torch.sum(torch.log(F.sigmoid(disc_class_tilde) + 1e-12))

        bce_dis_original = -torch.log(disc_class + 1e-3)
        # - torch.log(disc_class_tilde + 1e-12)
        bce_dis_predicted = -torch.log(1 - disc_class_tilde + 1e-3)
        bce_dis_sampled = -torch.log(1 - disc_class_sampled + 1e-3)

        return reconle, kl, bce_dis_original, bce_dis_predicted, bce_dis_sampled
